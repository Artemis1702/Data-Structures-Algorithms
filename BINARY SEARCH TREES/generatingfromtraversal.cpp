...
//actually we need pre-order+in-order but inorder traversal is just the elements in sorted order
//thus pre order or post order is alone enough
//but without inorder also BST can be generated, only pre -order or post-order is enough
...
void BST::createFromPreorder(int *pre, int n)
{
  ...
  while (i < n)
  {
    ...
    if (pre[i] < p->data)
    {
      ...
      t->rchild = nullptr;
      p->lchild = t;
      stk.push(p); //push only the left child
      p = t;
    }
    else
    {
      // Right child cases
      //when it is right child we dont push the address
