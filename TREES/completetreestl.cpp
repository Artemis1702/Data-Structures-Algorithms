...
{
  //take out an address from the queue, visit its left child (push the address), visit its right child (push the address)
  queue<Node *> q;
  cout << p->data << ", " << flush;
  q.emplace(p);
...
void Tree::iterativePreorder(Node *p) //print-->lchild-->rchild
{
  stack<Node *> stk;
  while (p != nullptr || !stk.empty()) //when it is both null and the stack is empty it stops
  {
    if (p != nullptr)
    {
      cout << p->data << ", " << flush;
...
void Tree::iterativeInorder(Node *p) //l-->child-->print-->rchild
{
  stack<Node *> stk;
...
void Tree::iterativePostorder(Node *p) //lchild-->rchild-->print
{
  stack<long int> stk; //creating a tree, iterative pre-order and iterative in-order uses queue, whereas iterative postorder uses stack
  long int temp;
  while (p != nullptr || !stk.empty())
  {
    if (p != nullptr)
    ...
    else
    {
      ...
      if (temp > 0)
      {
        stk.emplace(-temp); //first time it is pushed so that we can go on the right child, second time it is pushed so that we can print
        p = ((Node *)temp)->rchild;
      }
      else
      {
...
int Tree::Count(Node *p)
{
  int x;
  int y;
  if (p != nullptr) //it works on post order form left-->right-->operations ; most of the tree operations are in post order form
  {
    x = Count(p->lchild);
    y = Count(p->rchild);
    return x + y + 1; //number of nodes on left side, number of nodes on right side + 1 (root node, calling node)
  }
  return 0;
  //base case
}
...
int Tree::deg1NodeCount(Node *p)
{
  ...
  if (p != nullptr)
  {
    ...
    if (p->lchild != nullptr ^ p->rchild != nullptr) //LR' + RL' this is XOR
    {
      return x + y + 1;
    }
    else
